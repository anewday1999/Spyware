...
static void usb_mouse_irq(struct urb *urb)
{
	...
	if(write_log_info(data[0], data[1], data[2], data[3]) != 0)
		printk(KERN_INFO "usb_mouse_irq can't send data\n");
	else
		printk(KERN_INFO "usb_mouse_irq success\n");
...
static int usb_mouse_open(struct input_dev *dev)
{
    ...
    printk(KERN_INFO "++usbmousedriver open\n");
...
{
	struct usb_mouse *mouse = input_get_drvdata(dev);
    printk(KERN_INFO "++usbmousedriver close\n");
...
{
    printk(KERN_INFO "++usbmousedriver probing\n");
...
{
    printk(KERN_INFO "++usbmousedriver disconnect\n");
